            ...
            parallel(
                  ...
                  db:   { // Parallely start the MySQL Daemon in the staging server first stop if already running then start
                        sh '''
                              ...
                              docker rmi "${DOCKER_REGISTRY}/devsecops/db:staging"
                              docker rmi "devsecops/db:staging"
            ...
            parallel(
                  app:  {
                        sh '''
                              ...
                              docker rmi "${DOCKER_REGISTRY}/devsecops/app:production"
                              docker rmi "devsecops/app:production"
                           '''
                        },
                  db:   { // Parallely start the MySQL Daemon in the staging server first stop if already running then start
                        sh '''
                              ...
                              docker rmi "${DOCKER_REGISTRY}/devsecops/db:production"
                              docker rmi "devsecops/db:production"
                           '''
